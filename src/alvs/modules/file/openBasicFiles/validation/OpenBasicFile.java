 ...
 */
package alvs.modules.file.openBasicFiles.validation;
...
public class OpenBasicFile implements ALVSModule, TaskListener, ActionListener {
    ...
    public void initModule() {
        ...
        desktop.addMenuItem(ALVSMenu.FILE, "Open validation File..",
                "Opens validation of file", KeyEvent.VK_O, this, null, null);
    }

    public void taskStarted(Task task) {
        logger.info("Running validation Files");
    }

    public void taskFinished(Task task) {
        if (task.getStatus() == TaskStatus.FINISHED) {
            logger.info("Finished validation Files on " + ((OpenBasicFileTask) task).getTaskDescription());
        }

        if (task.getStatus() == TaskStatus.ERROR) {

            String msg = "Error while validation Files on .. " + ((OpenBasicFileTask) task).getErrorMessage();
            logger.severe(msg);
